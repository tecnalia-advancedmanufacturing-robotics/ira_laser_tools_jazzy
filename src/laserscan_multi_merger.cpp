    ...
    !equal(tmp_input_topics.begin(), tmp_input_topics.end(), input_topics.begin()))
  {
    ...
    if (input_topics.size() > 0) {
      ...
      for (uint i = 0; i < input_topics.size(); ++i) {
        ...
        std::function<void(const sensor_msgs::msg::LaserScan::SharedPtr)> callback = std::bind(
  ...
  std::string topic)
{
  ...
  bool canTransform = tf_buffer_->canTransform(
    destination_frame.c_str(), scan->header.frame_id.c_str(), time_point, tf2::durationFromSec(
      1.0));
  if (!canTransform) {
    RCLCPP_WARN(
      this->get_logger(),
